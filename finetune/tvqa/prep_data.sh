...
parallel -j $(nproc --all) --will-cite "python3 prep_data_mustard.py -fold {1} -num_folds ${NUM_FOLDS} " ::: $(seq 0 $((${NUM_FOLDS}-1)))
parallel -j $(nproc --all) --will-cite "python3 prep_data_mustard.py -fold {1} -num_folds ${NUM_FOLDS_VAL} -split=val " ::: $(seq 0 $((${NUM_FOLDS_VAL}-1)))
