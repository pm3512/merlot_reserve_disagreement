...
for idx, row in csv.iterrows():
    if idx % args.num_folds != args.fold:
        continue
    item = {}
    item['ts'] = (max(0, row['end'] - 29), float(row['end']))
...
def parse_item(item):
    ...
    language_item = language_sdk[int(item['vid_name'])]
    words = []
    for feat in language_item['context_features']:
        words.extend(feat)
    words.append('<|unused0|>')
    words.extend(language_item['punchline_features'])
    timestamps = []
    for feat in language_item['context_intervals'].tolist():
        timestamps.extend(feat)
    timestamps.append([language_item['punchline_intervals'][0][0], language_item['punchline_intervals'][0][0]])
    timestamps.extend(language_item['punchline_intervals'])
    start_times = list(map(lambda x: x[0], timestamps))
    end_times = list(map(lambda x: x[1], timestamps))
